...
def construct_asset_url(base_url, category, asset_id, name):
    """Centralized helper to generate canonical asset URLs."""
    slug = slugify(name)
    return f"{base_url.rstrip('/')}/view/{category}/{asset_id}/{slug}"
...
def clean_asset_list(assets):
    ...
    for asset in assets:
        ...
        # Add view_url for SEO and Frontend Routing
        cat = asset.get('category_type') or asset.get('category') or 'image'
        slug = slugify(asset.get('name', ''))
        asset['view_url'] = f"/view/{cat}/{asset.get('id')}/{slug}"
...
TRACKER_DB = os.path.join(DB_FOLDER, 'tracker.db')
...
# Initialize Gemini Flash / Imagen 3 Generator
GEMINI_API_KEY = os.environ.get('GEMINI_API_KEY') or os.environ.get('GOOGLE_API_KEY')
gemini_generator = GeminiGenerator(api_key=GEMINI_API_KEY, temp_folder=TEMP_FOLDER)
...
queue_manager = UploadQueueManager(visual_recognizer, temp_storage_path=QUEUE_STORAGE, rate_limit_seconds=1.0, vertex_generator=vertex_generator, gemini_generator=gemini_generator)
...
def load_translations():
    """Loads JSON translation files from the 'static/locales' directory."""
    global TRANSLATIONS
    messages_dir = os.path.join(os.path.dirname(__file__), 'static', 'locales')
    os.makedirs(messages_dir, exist_ok=True)
    
    # Map language codes to the specific filenames provided
    file_mapping = {
        'en': 'english.json',
        'es': 'espagnol.json',
        'pt': 'Brazilian Portuguese.json',
        'fr': 'fr.json',
        'de': 'german.json'
    }
...
def track_visitor():
    """Generates guest_id and session_id, and logs new visitors/sessions."""
    
    # 1. Ensure guest_id exists
    ...
    is_bot = 1 if any(x in ua_lower for x in ['bot', 'crawl', 'spider', 'google', 'bing', 'yandex', 'slurp', 'facebook', 'whatsapp', 'preview', 'curl', 'wget', 'python', 'java', 'http', 'opensiteexplorer', 'lighthouse', 'headless', 'scraper', 'checker']) else 0

    # We only want to write to DB if it's a new guest or new session to avoid DB spam on every request.
    if is_new_guest or is_new_session:
        try:
            ...
            now = time.time()
            if is_new_guest:
                # Basic Geolocation using ip-api.com (Free, No API key)
                country = ''
                city = ''
                try:
                    # Only look up if not a local/private IP
                    if ip_address and not ip_address.startswith(('127.', '192.168.', '10.', '172.16.', '172.17.', '172.18.', '172.19.', '172.2', '172.3', '::1')):
                        resp = requests.get(f'http://ip-api.com/json/{ip_address}', timeout=2)
                ...
                cursor.execute(
                    "INSERT OR IGNORE INTO visitors (guest_id, ip_address, country, city, user_agent, first_visit_date, user_id, is_bot) VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
                    (guest_id, ip_address, country, city, user_agent, now, session.get('user_id', None), is_bot)
                )
            else:
                # Returning visitor: 
                # 1. Update user_id if they are now logged in but weren't before
                user_id = session.get('user_id')
                if user_id:
                    cursor.execute("UPDATE visitors SET user_id = ? WHERE guest_id = ? AND user_id IS NULL", (user_id, guest_id))
                
                # 2. Backfill geolocation if missing
                try:
                    cursor.execute("SELECT country, city FROM visitors WHERE guest_id = ?", (guest_id,))
                    geo_row = cursor.fetchone()
                    if geo_row and (not geo_row[0] or geo_row[0] == ''):
                        if ip_address and not ip_address.startswith(('127.', '192.168.', '10.', '172.16.', '172.17.', '172.18.', '172.19.', '172.2', '172.3', '::1')):
                            resp = requests.get(f'http://ip-api.com/json/{ip_address}', timeout=2)
                            if resp.status_code == 200:
                                geo_data = resp.json()
                                if geo_data.get('status') == 'success':
                                    cursor.execute("UPDATE visitors SET country = ?, city = ?, ip_address = ? WHERE guest_id = ?", 
                                                   (geo_data.get('country', ''), geo_data.get('city', ''), ip_address, guest_id))
                except Exception as b_err:
                    print(f"Backfill geo error: {b_err}")
            
            if is_new_session:
                cursor.execute(
                    "INSERT OR IGNORE INTO sessions (session_id, guest_id, start_time, last_ping, duration_seconds) VALUES (?, ?, ?, ?, ?)",
                    (session_id, guest_id, now, now, 0)
                )
...
def before_request():
    ...
    # Apply tracking
    # Skip tracking for static files to save DB writes
    if not request.path.startswith(('/static/', '/uploads/', '/api/admin/')):
        track_visitor()
...
def add_performance_headers(response):
    """Sets headers for caching and security."""
    content_type = response.content_type
    
    # 1. Aggressive Caching for Static Assets (1 year)
    if request.path.startswith('/static/') or (content_type and any(t in content_type for t in ['image/', 'text/css', 'application/javascript', 'font/'])):
        response.headers['Cache-Control'] = 'public, max-age=31536000, immutable'
    
    # 2. No-Cache for Dynamic HTML
...
def init_tracker_db():
    """Initializes the tracking database."""
    try:
        conn = sqlite3.connect(TRACKER_DB)
        conn.execute("PRAGMA journal_mode=WAL;")
        cursor = conn.cursor()
        ...
        cursor.execute('''
            CREATE TABLE IF NOT EXISTS sessions (
                ...
                FOREIGN KEY(guest_id) REFERENCES visitors(guest_id)
            )
        ''')
        ...
        # Optimization: Add indexes to prevent lag on analytics dashboard
        cursor.execute("CREATE INDEX IF NOT EXISTS idx_visitors_date ON visitors(first_visit_date)")
        cursor.execute("CREATE INDEX IF NOT EXISTS idx_sessions_guest_id ON sessions(guest_id)")
        cursor.execute("CREATE INDEX IF NOT EXISTS idx_downloads_guest_id ON downloads_log(guest_id)")
        cursor.execute("CREATE INDEX IF NOT EXISTS idx_views_guest_id ON views_log(guest_id)")
        
        # Migration for user tracking
        try:
            cursor.execute("ALTER TABLE visitors ADD COLUMN user_id INTEGER")
        except sqlite3.OperationalError:
            pass # Column already exists
            
        # Migration for bot tracking
        try:
            cursor.execute("ALTER TABLE visitors ADD COLUMN is_bot INTEGER DEFAULT 0")
        except sqlite3.OperationalError:
            pass 
        ...
        print(f"Error initializing tracker db: {e}")

init_tracker_db()

# --- Indexing History Helper (Moved up for Analytics) ---
INDEXING_HISTORY_FILE = os.path.join(DB_FOLDER, 'indexing_history.json')

# Cache to prevent reading disk on every analytics poll
indexing_history_cache = {'data': {}, 'last_mtime': 0}

def load_indexing_history():
    """Loads the list of already submitted URLs."""
    global indexing_history_cache
    try:
        if not os.path.exists(INDEXING_HISTORY_FILE):
            return {}
            
        mtime = os.path.getmtime(INDEXING_HISTORY_FILE)
        if mtime > indexing_history_cache['last_mtime']:
            with open(INDEXING_HISTORY_FILE, 'r') as f:
                indexing_history_cache['data'] = json.load(f)
            indexing_history_cache['last_mtime'] = mtime
            
        return indexing_history_cache['data']
    except:
        return {}

@app.route('/api/track/ping', methods=['POST'])
def track_session_ping():
    """Receives a heartbeat from the frontend to update session duration."""
    ...
    try:
        ...
        if row:
            ...
            cursor.execute("UPDATE sessions SET last_ping = ?, duration_seconds = ? WHERE session_id = ?",
                           (now, duration, session_id))
                           
        # Link user_id if they logged in mid-session
        if 'guest_id' in session and 'user_id' in session:
            cursor.execute("UPDATE visitors SET user_id = ? WHERE guest_id = ? AND user_id IS NULL",
        ...
        print(f"Tracking error ping: {e}")
        return jsonify({"success": False, "message": str(e)}), 500

@app.route('/api/track/download', methods=['POST'])
def track_asset_download():
    """Logs an asset download from the frontend."""
    guest_id = session.get('guest_id')
    user_id = session.get('user_id')
    
    if not guest_id:
        return jsonify({"success": False, "message": "No guest ID"}), 400
    ...
    if not asset_id:
        return jsonify({"success": False, "message": "Missing asset_id"}), 400
        
    try:
        conn = sqlite3.connect(TRACKER_DB)
        cursor = conn.cursor()
        cursor.execute(
            "INSERT INTO downloads_log (guest_id, user_id, asset_id, category, timestamp) VALUES (?, ?, ?, ?, ?)",
            (guest_id, user_id, asset_id, category, time.time())
        )
        conn.commit()
        conn.close()
        
        # Also update the main DB's download count
        db_name = DB_MAPPING.get(category)
        if db_name:
            try:
                ...
                # Update count
                main_cursor.execute("UPDATE uploads SET downloads = downloads + 1 WHERE id = ?", (asset_id,))
                
                # Fetch owner for notification
                main_cursor.execute("SELECT user_id, name FROM uploads WHERE id = ?", (asset_id,))
                ...
                print(f"Failed to update main db download count: {e}")
                
        return jsonify({"success": True})
    except Exception as e:
        print(f"Tracking error download: {e}")
        return jsonify({"success": False, "message": str(e)}), 500

@app.route('/api/admin/analytics_data', methods=['GET'])
def admin_analytics_data():
    """Serves tracking data to the admin dashboard."""
    ...
    try:
        ...
        cursor.execute("SELECT COUNT(*) as count FROM visitors WHERE first_visit_date > ? AND (is_bot = 0 OR is_bot IS NULL)", (cutoff,))
        total_visitors = cursor.fetchone()['count']
        
        # 1.5 Total Bots in period
        cursor.execute("SELECT COUNT(*) as count FROM visitors WHERE first_visit_date > ? AND is_bot = 1", (cutoff,))
        total_bots = cursor.fetchone()['count']

        # 2. Advanced Trend Data Aggregation
        # Decide granularity: if period <= 2 days, use hourly. Else daily.
        granularity = 'hourly' if days <= 2 else 'daily'
        # SQLite format based on granularity
        sql_format = '%Y-%m-%d %H:00' if granularity == 'hourly' else '%Y-%m-%d'
        
        # We will build a unified mapping of { time_bucket: {visitors: X, downloads: Y, views: Z, generations: W} }
        trend_map = {}
        
        # Helper to initialize bucket if missing
        def init_bucket(bucket):
            if bucket not in trend_map:
                trend_map[bucket] = {'time': bucket, 'visitors': 0, 'downloads': 0, 'views': 0, 'generations': 0}
        
        # A. Get Visitors trend
        cursor.execute(f"SELECT strftime('{sql_format}', first_visit_date, 'unixepoch', 'localtime') as bucket, COUNT(*) as count FROM visitors WHERE first_visit_date > ? AND (is_bot = 0 OR is_bot IS NULL) GROUP BY bucket", (cutoff,))
        for row in cursor.fetchall():
            init_bucket(row['bucket'])
            trend_map[row['bucket']]['visitors'] = row['count']
            
        # B. Get Downloads trend
        cursor.execute(f"SELECT strftime('{sql_format}', timestamp, 'unixepoch', 'localtime') as bucket, COUNT(*) as count FROM downloads_log WHERE timestamp > ? GROUP BY bucket", (cutoff,))
        for row in cursor.fetchall():
            init_bucket(row['bucket'])
            trend_map[row['bucket']]['downloads'] = row['count']
            
        # C. Get Views trend
        cursor.execute(f"SELECT strftime('{sql_format}', timestamp, 'unixepoch', 'localtime') as bucket, COUNT(*) as count FROM views_log WHERE timestamp > ? GROUP BY bucket", (cutoff,))
        for row in cursor.fetchall():
            init_bucket(row['bucket'])
            trend_map[row['bucket']]['views'] = row['count']
            
        # D. Get Generations trend (from Generated DB)
        try:
            gen_conn_trend = sqlite3.connect(GENERATED_DB)
            gen_cursor = gen_conn_trend.cursor()
            gen_cursor.execute(f"SELECT strftime('{sql_format}', created_at, 'unixepoch', 'localtime') as bucket, COUNT(*) as count FROM user_generations WHERE created_at > ? GROUP BY bucket", (cutoff,))
            for r_bucket, r_count in gen_cursor.fetchall():
                init_bucket(r_bucket)
                trend_map[r_bucket]['generations'] = r_count
            gen_conn_trend.close()
        except Exception as e:
            print(f"Trend Generations Error: {e}")

        # Convert map to sorted list
        trend_data = sorted(trend_map.values(), key=lambda x: x['time'])
        
        # 3. Average session duration
        cursor.execute("SELECT AVG(duration_seconds) as avg_duration FROM sessions WHERE start_time > ?", (cutoff,))
        avg_row = cursor.fetchone()
        avg_duration = round(avg_row['avg_duration'] or 0, 1)
        
        # 4. Total downloads
        cursor.execute("SELECT COUNT(*) as count FROM downloads_log WHERE timestamp > ?", (cutoff,))
        total_downloads = cursor.fetchone()['count']
        
        # 5. Total views in period
        cursor.execute("SELECT COUNT(*) as count FROM views_log WHERE timestamp > ?", (cutoff,))
        total_views_period = cursor.fetchone()['count']
        
        # 6. Recent visitors (for table)
        cursor.execute('''
            SELECT v.guest_id, v.user_id, v.ip_address, v.country, v.city, v.user_agent, v.first_visit_date, 
            SUM(s.duration_seconds) AS total_duration, COUNT(d.id) AS downloads
            FROM visitors v
            LEFT JOIN sessions s ON v.guest_id = s.guest_id
            LEFT JOIN downloads_log d ON v.guest_id = d.guest_id
            WHERE (v.is_bot = 0 OR v.is_bot IS NULL)
            GROUP BY v.guest_id
            ORDER BY v.first_visit_date DESC LIMIT 50
        ''')
        recent_visitors_raw = [dict(row) for row in cursor.fetchall()]
        conn.close()
        
        # 7. Enrich visitors with Users and AI data
        recent_visitors = []
        gen_conn = sqlite3.connect(GENERATED_DB)
        users_conn = sqlite3.connect(USERS_DB)
        
        for v in recent_visitors_raw:
            v_enriched = dict(v)
            v_enriched['generations'] = 0
            v_enriched['username'] = None
            v_enriched['avatar'] = None
            
            # If the user is signed in, fetch user details and generations
            user_id = v.get('user_id')
            if user_id:
                # Get avatar and username
                try:
                    c_users = users_conn.cursor()
                    c_users.execute("SELECT username, avatar FROM users WHERE id = ?", (user_id,))
                    u_row = c_users.fetchone()
                    if u_row:
                        v_enriched['username'] = u_row[0]
                        v_enriched['avatar'] = u_row[1]
                except Exception as e:
                    print(f"Error fetching user for visitor: {e}")
                
                   # Get total AI generations for this user in the period
                try:
                    c_gen = gen_conn.cursor()
                    c_gen.execute("SELECT COUNT(*) FROM user_generations WHERE user_id = ? AND created_at > ?", (user_id, cutoff))
                    g_row = c_gen.fetchone()
                    if g_row:
                        v_enriched['generations'] = g_row[0]
                except Exception as e:
                   print(f"Error fetching gens for visitor: {e}")
            
            recent_visitors.append(v_enriched)
            
        gen_conn.close()
        users_conn.close()
        
        # Fetch Indexing Stats (from admin_tool.py system)
        # indexing_history = load_indexing_history() # Now defined above
        indexing_history = load_indexing_history()
        total_indexed = len(indexing_history)
...
def index():
    ...
    # Determine auto-open modals based on route
    path = request.path.rstrip('/')
    open_settings = path in ['/settings', '/privacy', '/terms']
    open_about = path == '/about'
    ...
    if path == '/about': 
        target_tab = 'about'
        seo_title = "About Asanji Bruno - Founder of wallpepersphere"
        seo_description = "Meet Asanji Bruno, the Cameroonian developer and visionary behind wallpepersphere. Learn about our mission to democratize creativity with AI and 4K assets."
    elif path == '/privacy': 
        target_tab = 'privacy'
        seo_title = "Privacy Policy - wallpepersphere"
        seo_description = "Read our Privacy Policy to understand how wallpepersphere collects, uses, and protects your data. Transparency and security are our top priorities."
    elif path == '/terms': 
        target_tab = 'terms'
        seo_title = "Terms of Service - wallpepersphere"
        seo_description = "Review the Terms of Service for wallpepersphere. Understand the rules, usage rights, and guidelines for using our platform and assets."
    elif path == '/settings':
        seo_title = "Settings - wallpepersphere"
        if not session.get('user_id'): target_tab = 'appearance'
        else: target_tab = 'account'

    # If opening settings/about, fetch user data for SSR
    user_settings = {}
    if open_settings or open_about:
        user_settings = get_user_settings_data(session.get('user_id'))
...
def get_user_settings_data(user_id):
    """Helper to fetch user settings data from the database."""
    user_data = {
        'username': '',
        'email': '',
        'avatar': None,
    ...
    try:
        ...
        cursor.execute("SELECT username, email, avatar, bio, website, instagram, twitter, contact_email FROM users WHERE id = ?", (user_id,))
        row = cursor.fetchone()
        if row:
            user_data = dict(row)
        conn.close()
...
def settings_template():
    ...
    user_data = get_user_settings_data(session.get('user_id'))
...
def generate_image_route():
    ...
    ref_image_base64 = data.get('ref_image')
    ...
    if ref_image_base64:
        try:
            ...
            if ',' in ref_image_base64:
                header, encoded = ref_image_base64.split(',', 1)
            else:
                encoded = ref_image_base64
            file_data = base64.b64decode(encoded)
            filename = f"gen_ref_{user_id}_{int(time.time())}.jpg"
            ref_image_path = os.path.join(TEMP_FOLDER, filename)
            with open(ref_image_path, 'wb') as f:
                f.write(file_data)
        except Exception as e:
            print(f"Error handling ref_image base64 in generate: {e}")
...
def edit_image_route():
    """Handles an existing image edit request."""
    user_id = session.get('user_id')
    if not user_id:
        if 'guest_id' not in session:
            session['guest_id'] = -random.randint(100000, 999999999)
    ...
    prompt = data.get('prompt')
    aspect_ratio = data.get('aspect_ratio', '1:1')
    model = data.get('model', 'flash') # Default to flash for edit
    ref_image_base64 = data.get('ref_image')
    
    if not prompt or not ref_image_base64:
        return jsonify({'success': False, 'message': 'Prompt and reference image are required'}), 400

    # Save base64 to a temporary file
    try:
        import base64
        # Assuming the format "data:image/jpeg;base64,/9j/4AAQ..."
        if ',' in ref_image_base64:
            header, encoded = ref_image_base64.split(',', 1)
        else:
            encoded = ref_image_base64
        ...
        file_data = base64.b64decode(encoded)
        filename = f"edit_ref_{user_id}_{int(time.time())}.jpg"
        ref_image_path = os.path.join(TEMP_FOLDER, filename)
        
        with open(ref_image_path, 'wb') as f:
            f.write(file_data)
    except Exception as e:
        print(f"Error handling ref_image base64: {e}")
    ...
    try:
        ...
        print(f"Error logging edit generation request: {e}")
...
def check_task(task_id):
    ...
    if task['status'] == 'completed':
        ...
        # For generation and edit tasks, include the task type so the frontend knows how to handle it
        if task.get('type') in ('generate', 'edit'):
...
def publish_file():
    ...
    for item in loop_source:
        # FIX: Normalize string items (filenames) to dicts so they can be processed
        if isinstance(item, str):
            item = {'filename_original': item}
        
        fname = item['filename_original']
    ...
    if success_count > 0:
        ...
        try:
            # Use PRODUCTION_URL if set, otherwise fallback to request host
            host_url = PRODUCTION_URL if PRODUCTION_URL else request.url_root.rstrip('/')
            threading.Thread(target=auto_submit_to_google, args=(published_items, host_url)).start()
        except Exception as e:
            print(f"Error triggering auto-indexing: {e}")
...
def uploaded_file(filename):
    ...
    if request.args.get('proxy') == '1' and s3_client and R2_BUCKET_NAME:
        try:
            ...
            # IMPORTANT: Do not fall through on proxy failure. Return an error.
            return "Error proxying file from cloud storage (CORS?)", 500

    # 1. Priority: Generate Presigned URL (for direct linking, not for canvas)
...
def get_assets(category):
    ...
    # Fix: Use X-Forwarded-For to get real IP behind proxies (Render/Cloudflare)
    ...
    # Increased threshold to 1000 to prevent false positives on heavy loads
    if current_load + limit > 1000:
...
def get_similar_assets(category, asset_id):
    ...
    if limit > 50: limit = 50 # Cap limit
    ...
    try:
        ...
        # Now, get random assets from the same category, including user info
        sql = f"""
            SELECT t.id, t.name, t.link_small, t.link_tiny, t.category, t.resolution, t.user_id,
                   '{category}' as category_type, u.username, u.avatar,
                   EXISTS(SELECT 1 FROM users_db.user_likes WHERE user_id = ? AND asset_id = t.id AND category = ?) as is_liked,
                   EXISTS(SELECT 1 FROM users_db.user_saves WHERE user_id = ? AND asset_id = t.id AND category = ?) as is_saved
            FROM uploads t
        ...
        params = [user_id, category, user_id, category, asset_category, asset_id, limit]
        cursor.execute(sql, params)
...
def perform_search_logic(query, category_filter, quality_filter, size_filter, sort_order, user_id, limit=None):
    asset_results = []
    user_results = []

    # --- USER SEARCH LOGIC (10% priority means they appear first) ---
    if query:
        try:
            with sqlite3.connect(USERS_DB) as conn:
                conn.row_factory = sqlite3.Row
                cursor = conn.cursor()
                user_search_term = f"%{query}%"
                # Limit to 3 user results to not overwhelm the search
                cursor.execute("SELECT id, username, avatar FROM users WHERE username LIKE ? ORDER BY CASE WHEN username LIKE ? THEN 1 ELSE 2 END, username LIMIT 3", (user_search_term, query))
                found_users = cursor.fetchall()

                for user_row in found_users:
                    user_id_found = user_row['id']
                    stats = user_manager.get_follow_stats(user_id_found)
                    
                    recent_images = []
                    # Fetch from image DB first as it's primary
                    with sqlite3.connect(DB_MAPPING['image']) as conn_img:
                        cursor_img = conn_img.cursor()
                        cursor_img.execute("SELECT link_small FROM uploads WHERE user_id = ? ORDER BY upload_date DESC LIMIT 3", (user_id_found,))
                        recent_images.extend([row[0] for row in cursor_img.fetchall()])
                    
                    user_data = {
                        'type': 'user', # Special type for the frontend
                        'id': user_id_found,
                        'username': user_row['username'],
                        'avatar': user_row['avatar'],
                        'followers': stats.get('followers', 0),
                        'is_following': user_manager.is_following(user_id, user_id_found) if user_id and user_id > 0 else False,
        ...
        try:
            ...
            if search_terms:
                ...
                for term in search_terms:
                    ...
                    wildcard = f"%{term}%"
                    # Each term must be present, so we add an AND clause for each term.
                    sql += " AND (t.name LIKE ? OR t.description LIKE ? OR t.key_word LIKE ? OR t.category LIKE ? OR t.color_code LIKE ? OR u.username LIKE ?)"
                    params.extend([wildcard, wildcard, wildcard, wildcard, wildcard, wildcard])
    ...
    for asset in asset_results:
        ...
        for term in search_terms: # This loop is for assets, not users
    ...
    # Sort asset results based on relevance and user preference
    ...
    if query:
        if sort_order == 'popular':
            asset_results.sort(key=lambda x: (x['relevance_score'], x.get('views', 0), x.get('likes', 0)), reverse=True)
        else:
            asset_results.sort(key=lambda x: (x['relevance_score'], x.get(primary_sort_key, 0)), reverse=True)
    else:
        if sort_order == 'popular':
            asset_results.sort(key=lambda x: (x.get('views', 0), x.get('likes', 0)), reverse=True)
        else:
            asset_results.sort(key=lambda x: x.get(primary_sort_key, 0), reverse=True)

    results = user_results + asset_results
    ...
    if len(results) < MIN_RESULTS:
        ...
        for cat in fallback_cats:
            ...
            try:
                ...
                """
                cursor.execute(sql, (user_id, needed * 3))
                rows = cursor.fetchall()
                conn.close()
    ...
    # Final cleaning pass
    cleaned = clean_asset_list(results)
    if limit:
...
def search_assets():
    ...
    # --- FIX: Handle explicit user search (Trending Library) ---
    ...
    if req_type == 'user':
        ...
        try:
            with sqlite3.connect(USERS_DB) as conn:
                ...
                for row in rows:
                    uid = row['id']
                    stats = user_manager.get_follow_stats(uid)
                    is_following = False
                    if user_id:
                        try:
                            uid_int = int(user_id)
                            if uid_int > 0:
                                is_following = user_manager.is_following(uid_int, uid)
                        except: pass
                    
                    # Get recent images for card background
                    recent = []
                    try:
                        with sqlite3.connect(DB_MAPPING['image']) as c_img:
                            cur_img = c_img.cursor()
                            cur_img.execute("SELECT link_small FROM uploads WHERE user_id = ? ORDER BY upload_date DESC LIMIT 3", (uid,))
                            recent = [r[0] for r in cur_img.fetchall()]
                    except: pass
            ...
            return jsonify({'success': True, 'assets': clean_asset_list(users)})
        except Exception as e:
            print(f"User search error: {e}")
            return jsonify({'success': False, 'message': str(e)}), 500
    # -----------------------------------------------------------
    
    results = perform_search_logic(query, category_filter, quality_filter, size_filter, sort_order, user_id, limit=limit)
...
def save_generated_image():
    ...
    try:
        ...
        return jsonify({
            'success': True, 
            'message': 'Image saved to your private library.',
            'asset': {
                'id': new_id,
                'link_original': r2_key,
                'link_small': r2_key,
                'name': 'AI Generation',
...
@app.route('/api/user/generations')
def get_user_generations_api():
    """API endpoint to fetch private generations/renders for the current user."""
    if 'user_id' not in session:
        return jsonify({'success': False, 'message': 'AUTH_REQUIRED'}), 401
    ...
    try:
        ...
        # Map to common asset format for editor library
        assets = []
        ...
        for row in rows:
            assets.append({
                ...
                'link_small': row['r2_key'],
                'link_original': row['r2_key'],
...
def view_user_profile(user_id):
    """
    SEO Landing Page for a specific user profile.
    Uses ID to ensure uniqueness even if usernames change or are duplicates.
    """
    ...
    # We don't want to pass the password hash to the template.
    public_user_data = {
        'id': user['id'],
    ...
    # This logic is for the logged-in user's UI.
    has_unread = False
    theme_color = '#0a84ff'
    theme_mode = 'light'
    if 'user_id' in session:
        try:
            ...
            print(f"Error checking notifications/prefs on profile page: {e}")

    # Render home but pass the user for SEO tags and JS auto-open
    return render_template('home.html', view_profile_user=public_user_data, r2_domain=R2_DOMAIN, has_unread=has_unread, theme_color=theme_color, theme_mode=theme_mode, current_page=1)

@app.route('/@<string:username>')
def view_user_profile_legacy(username):
    """Legacy route for username-based links. Redirects to ID-based route."""
    user = user_manager.get_user_by_username(username)
    if user:
        return redirect(url_for('view_user_profile', user_id=user['id']), code=301)
    return redirect(url_for('index'))
    ...
    for cat, db_path in DB_MAPPING.items():
        try:
            ...
            for row in rows:
                ...
                loc = construct_asset_url(host, cat, row[0], row[1])
...
def profile_template():
    ...
    if user_id:
        ...
        # Fetch Follow Stats
        stats = user_manager.get_follow_stats(user_id)
        followers_count = stats['followers']
        following_count = stats['following']

    return render_template('profile.html', username=username, avatar_letter=avatar_letter, avatar=avatar, total_likes=total_likes, total_views=total_views, total_downloads=total_downloads, latest_notification=latest_notification, latest_notification_type=latest_notification_type, r2_domain=R2_DOMAIN, role=role, followers_count=followers_count, following_count=following_count)

@app.route('/user-library/<int:user_id>')
def user_library(user_id):
    """
    Legacy route for user profiles. Permanently redirects to the new ID-based URL.
    """
    return redirect(url_for('view_user_profile', user_id=user_id), code=301)
...
def get_public_user_details(user_id):
    ...
    try:
        ...
        if row:
            ...
            # Get Follow Stats
            stats = user_manager.get_follow_stats(user_id)
            user_data['followers'] = stats['followers']
            user_data['following'] = stats['following']
            
            # Check if current user is following
            user_data['is_following'] = False
            if 'user_id' in session:
                user_data['is_following'] = user_manager.is_following(session['user_id'], user_id)
...
def track_view():
    ...
    try:
        ...
        # Log to Tracker DB (Analytics)
        try:
            guest_id = session.get('guest_id')
            if guest_id:
                t_conn = sqlite3.connect(TRACKER_DB)
                t_conn.execute("INSERT INTO views_log (guest_id, user_id, asset_id, category, timestamp) VALUES (?, ?, ?, ?, ?)",
                               (guest_id, session.get('user_id'), asset_id, category, time.time()))
                t_conn.commit()
                t_conn.close()
        except Exception as e:
            print(f"Tracker View Error: {e}")
...
def toggle_follow():
    ...
    try:
        # Ensure both IDs are integers to prevent type-related errors.
        follower_id_int = int(follower_id)
        target_id_int = int(target_id)
    except (ValueError, TypeError):
        return jsonify({'success': False, 'message': 'Invalid user ID format'}), 400

    # Use UserManager for logic, passing integer IDs to ensure type safety.
    result = user_manager.toggle_follow(follower_id_int, target_id_int)
    
    # Handle Notification separately in the controller (app.py)
    if result['success'] and result['action'] == 'followed':
        create_notification(target_id_int, f"{session.get('username', 'Someone')} started following you.", "system")
    ...
    for cat, db_path in DB_MAPPING.items():
        try:
            with sqlite3.connect(db_path) as conn:
                ...
                rows = cursor.fetchall()
                for row in rows:
                    url = construct_asset_url(host, cat, row[0], row[1])
                    urls.append(url)
        except: pass
...
def get_indexing_credentials():
    ...
    # Priority: GOOGLE_INDEXING_JSON (Specific) -> GCP_CREDENTIALS_JSON (Render) -> GOOGLE_CREDENTIALS_JSON (Generic)
    creds_json = os.environ.get('GOOGLE_INDEXING_JSON') or os.environ.get('GCP_CREDENTIALS_JSON') or os.environ.get('GOOGLE_CREDENTIALS_JSON')
    if creds_json:
        try:
...
def auto_submit_to_google(items, host):
    """Background task to submit new uploads to Google Indexing."""
    # Wait for R2 propagation and DB consistency (SEO Best Practice)
    time.sleep(10)

    credentials = get_indexing_credentials()
    if not credentials:
        print("Auto-Indexing: No credentials found.")
        return

    # Sync with Admin System History
    history = load_indexing_history()
    updated = False
    ...
    try:
        ...
        for item in items:
            try:
                url = construct_asset_url(host, item['category'], item['id'], item['name'])
                content = {"url": url, "type": "URL_UPDATED"}
                service.urlNotifications().publish(body=content).execute()
                print(f"Auto-Indexed: {url}")
                
                # Record in history for Admin Dashboard
                history[url] = time.time()
                updated = True
            except Exception as e:
                print(f"Failed to auto-index {item.get('name')}: {e}")
        
        if updated:
            save_indexing_history(history)
    except Exception as e:
        print(f"Auto-Indexing Service Error: {e}")

@app.route('/api/admin/indexing/submit', methods=['POST'])
@admin_required
def admin_indexing_submit():
    """Submits a single URL to Google."""
    ...
    if not manual_url:
        return jsonify({'success': False, 'message': 'URL is required for manual submission.'}), 400
