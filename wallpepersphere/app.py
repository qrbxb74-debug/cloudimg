...
queue_manager = UploadQueueManager(visual_recognizer, temp_storage_path=QUEUE_STORAGE, rate_limit_seconds=5.0)

# Initialize Sitemap Manager
SITEMAP_FOLDER = os.path.join(DB_FOLDER, 'sitemaps')
# Define Site URL (Use env var or default to your domain)
SITE_URL = os.environ.get('APP_URL', 'https://wallpepersphere.com').rstrip('/')
sitemap_manager = SitemapManager(SITEMAP_FOLDER, SITE_URL)
...
def prepare_images_for_r2(src_path, base_filename):
    ...
    try:
        with Image.open(src_path) as img:
            ...
            rgb_im.thumbnail((1024, 1024))
            compress_and_save(rgb_im, small_path, 30)
    ...
    for item in loop_source:
        ...
        if isinstance(item, dict):
            try:
                ...
                # --- SITEMAP UPDATE ---
                # Add the new asset to the sitemap immediately
                try:
                    safe_slug = slugify(final_name)
                    page_url = f"{SITE_URL}/view/{category}/{cursor.lastrowid}/{safe_slug}"
                    
                    # Ensure we get the original filename safely
                    sitemap_img = item['filename_original'] if isinstance(item, dict) else item
                    
                    img_url = f"{R2_DOMAIN}/{sitemap_img}" if R2_DOMAIN else f"{SITE_URL}/uploads/{sitemap_img}"
                    lastmod = datetime.now().strftime('%Y-%m-%d')
                    sitemap_manager.add_asset(page_url, lastmod, img_url, final_name)
                except Exception as e:
...
def sitemap_index():
    ...
    try:
        return send_from_directory(SITEMAP_FOLDER, 'sitemap_index.xml', mimetype='text/xml')
    except Exception:
        return "Sitemap index not found. Please rebuild sitemaps.", 404

@app.route('/sitemaps/<path:filename>')
def serve_sitemap(filename):
    """Serves individual static sitemap files."""
    return send_from_directory(SITEMAP_FOLDER, filename, mimetype='text/xml')

@app.route('/robots.txt')
def robots():
    """Serves robots.txt to guide search engine crawlers."""
...
@app.route('/api/admin/rebuild_sitemap', methods=['POST'])
@admin_required
def admin_rebuild_sitemap():
    """Admin task to fully rebuild all sitemaps from the database."""
    def asset_generator():
        # Yields assets one by one to avoid memory issues
        for cat, db_path in DB_MAPPING.items():
            try:
                conn = sqlite3.connect(db_path)
                cursor = conn.cursor()
                # Select all assets
                cursor.execute("SELECT id, name, link_medium, link_original, upload_date FROM uploads ORDER BY upload_date DESC")
                ...
                while True:
                    ...
                    for row in rows:
                        asset_id, name, link_medium, link_original, upload_date = row
                        safe_slug = slugify(name)
                        
                        # Prefer original for sitemap (highest quality for Google)
                        img_file = link_original if link_original else link_medium
                        
                        # Handle case where DB might contain full URL (from bulk imports) vs Filename (from web uploads)
                        if img_file and (img_file.startswith('http://') or img_file.startswith('https://')):
                             img_url = img_file
                        else:
                             img_url = f"{R2_DOMAIN}/{img_file}" if R2_DOMAIN else f"{SITE_URL}/uploads/{img_file}"
                        
                        yield {
                            'page_url': f"{SITE_URL}/view/{cat}/{asset_id}/{safe_slug}",
                            'lastmod': time.strftime('%Y-%m-%d', time.localtime(upload_date)) if upload_date else None,
                            'image_url': img_url,
                            'title': name
                        }
                conn.close()
            except Exception as e:
                print(f"Error scanning {cat} for sitemap: {e}")

    try:
        count = sitemap_manager.rebuild(asset_generator())
        return jsonify({'success': True, 'message': f'Sitemaps rebuilt. Created {count} files.'})
    except Exception as e:
        return jsonify({'success': False, 'message': str(e)}), 500
