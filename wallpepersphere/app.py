            ...
            with sqlite3.connect(USERS_DB) as conn:
                ...
                for row in rows:
                    ...
                    if user_id and user_id > 0:
                        is_following = user_manager.is_following(user_id, uid)
...
def toggle_follow():
    ...
    if int(target_id) == int(follower_id):
        return jsonify({'success': False, 'message': 'Cannot follow yourself'}), 400

    # Use UserManager for logic
    result = user_manager.toggle_follow(follower_id, target_id)
    
    # Handle Notification separately in the controller (app.py)
    if result['success'] and result['action'] == 'followed':
        create_notification(target_id, f"{session.get('username', 'Someone')} started following you.", "system")
