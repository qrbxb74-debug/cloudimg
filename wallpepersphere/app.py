...
def index():
    ...
    open_settings = request.path.rstrip('/') == '/settings'
    open_about = request.path.rstrip('/') == '/about'
    open_privacy = request.path.rstrip('/') == '/privacy'
    open_terms = request.path.rstrip('/') == '/terms'
...
def get_similar_assets(category, asset_id):
    ...
    try:
        ...
        # Connect users database to join for username and avatar
        conn.execute(f"ATTACH DATABASE '{USERS_DB}' AS users_db")
        
        # Limit added via query param, defaulting to 12
        limit = request.args.get('limit', 12, type=int)

        # Now, get a few random assets from the same category along with user info
        cursor.execute(f"""
            SELECT t.id, t.name, t.link_small, t.category, t.resolution, 
            t.user_id, users.username, users.avatar, '{category}' as category_type
            FROM uploads t
            LEFT JOIN users_db.users users ON t.user_id = users.id
            WHERE t.category = ? AND t.id != ?
            ORDER BY RANDOM()
            LIMIT ?
        """, (asset_category, asset_id, limit))
