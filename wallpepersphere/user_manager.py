...
class UserManager:
    ...
    def toggle_follow(self, follower_id, target_id):
        ...
        cursor = conn.cursor()
        try:
            cursor.execute("SELECT 1 FROM user_follows WHERE follower_id = ? AND followed_id = ?", (f_id, t_id))
            if cursor.fetchone():
                cursor.execute("DELETE FROM user_follows WHERE follower_id = ? AND followed_id = ?", (f_id, t_id))
                action = 'unfollowed'
            else:
                cursor.execute("INSERT INTO user_follows (follower_id, followed_id, created_at) VALUES (?, ?, ?)", (f_id, t_id, time.time()))
                action = 'followed'
            conn.commit()
