        ...
        try:
            ...
            for p in priorities:
                for m_name in model_names:
                    ...
                    if p == clean_name or p in m_name:
                        print(f"Selected model: {m_name}")
                        return m_name
            
            # Fallback: Use the first available model
            if model_names:
                print(f"No priority model found. Using first available: {model_names[0]}")
                return model_names[0]
        except Exception as e:
            print(f"Error listing models: {e}")
        
        # Ultimate fallback if listing fails
        print("Model listing failed. Using fallback: gemini-1.5-flash")
