...
if not logger.handlers:
    fh = logging.FileHandler("queue_debug.log")
...
class UploadQueueManager:
    def __init__(self, visual_recognizer, temp_storage_path, rate_limit_seconds=4.0, vertex_generator=None, gemini_generator=None):
        """
        ...
        :param gemini_generator: Instance of GeminiGenerator to perform image generation.
    ...
    def add_to_queue(self, task_type, user_id, **kwargs):
        ...
        :param task_type: 'analyze', 'generate', or 'edit'.
        :param user_id: The ID of the user uploading the file.
        :param kwargs: For 'analyze', pass 'source_file_path'.
                       For 'generate', pass 'prompt' and 'aspect_ratio'.
                       For 'edit', pass 'prompt', 'aspect_ratio', and 'reference_image_path'.
        ...
        try:
            ...
            if task_type == 'analyze':
                ...
                task.update({
                    ...
                    'model': kwargs.get('model', 'vertex'),
                    'reference_image_path': kwargs.get('reference_image_path')
                })

            elif task_type == 'edit':
                if not self.gemini_generator:
                    raise ValueError("GeminiGenerator not configured in QueueManager.")
                prompt = kwargs.get('prompt')
                ref_path = kwargs.get('reference_image_path')
                if not prompt or not ref_path:
                    raise ValueError("Prompt and reference_image_path are required for edit task.")
                task.update({
                    'prompt': prompt,
                    'aspect_ratio': kwargs.get('aspect_ratio', '1:1'),
                    'reference_image_path': ref_path,
                    'model': 'flash'  # force Gemini for edit
                })
    ...
    def _handle_generation_processing(self, task):
        ...
        try:
            ...
            # Call the correct AI generator based on model type
            generator = self.vertex_generator
            if task.get('model') == 'flash':
                generator = self.gemini_generator
                
            if not generator:
                task['status'] = 'failed'
                task['error'] = 'GENERATOR_NOT_CONFIGURED'
                return
